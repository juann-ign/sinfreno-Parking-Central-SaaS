...
def login(
    ...
    # Construimos la info de la sucursal solo si existe
    sucursal_info = None
    if user.sucursal:
        sucursal_info = {
            "id": user.sucursal_id,
    ...
    return {
        ...
        "user_info": {
            ...
            "sucursal": sucursal_info  # Será None para el SuperAdmin
        }
    }
...
def discover_tenant(email: str, db: Session = Depends(dependencies.get_db)):
    ...
    # Si el usuario no existe O no tiene sucursal (es SuperAdmin)
    if not user or not user.sucursal:
        return {
            "logo": None, 
            "color": "#4f46e5", 
            "empresa": "Sinfreno"
